(module
  ...
  (elem (i32.const 1) $set_sink $void_call_roundtrip $nested_call_arg)
  (global $__funcidx_3 i32 (i32.const 1))
  (global $__funcidx_4 i32 (i32.const 2))
  (global $__funcidx_5 i32 (i32.const 3))
  ...
  (func $set_sink (export "set_sink") (param $p0 i32)
    (local $fp i32)
    (local $r0 i32)
  ...
  (func $void_call_roundtrip (export "void_call_roundtrip") (result i32)
    (local $fp i32)
    (local.set $fp (global.get $__stack_pointer))
    (call $set_sink (i32.const 37))
    (i32.load (global.get $__addr_2))
    (return)
  )
  (func $nested_call_arg (export "nested_call_arg") (param $p0 i32) (result i32)
    (local $fp i32)
    (local.set $fp (global.get $__stack_pointer))
    (global.set $__stack_pointer (i32.sub (local.get $fp) (i32.const 16)))
    (i32.store (i32.add (local.get $fp) (i32.const -4)) (local.get $p0))
    (call $set_sink (i32.add (i32.load (i32.add (local.get $fp) (i32.const -4))) (i32.const 5)))
    (i32.add (i32.load (global.get $__addr_2)) (i32.const 1))
    (global.set $__stack_pointer (local.get $fp))
    (return)
  )
)
